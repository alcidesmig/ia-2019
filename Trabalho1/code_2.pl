% Código que não sobe no fogo se não puder apagá-lo
...
andarFogo(I, J, Fogos) :-
	(
		not(pertence((I, NovoJ), Fogos))			% Verifica se não existe fogo
			;										% ou
		(		
			pertence((I, NovoJ), Fogos),			% Existe fogo 
...
s(
	...
	andarFogo(I, NovoJ, Fogos).				% Verifica se não existe fogo, ou existe e tem carga para apagá-lo
